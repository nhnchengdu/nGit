...
//buid event in configer
//"C:\Program Files\Microsoft Visual Studio 2008 SDK\VisualStudioIntegration\Tools\bin\RegPkg.exe" /regfile:SccProvider.reg /codebase "$(TargetPath)"
...
namespace Nhn.Git.SourceControl.Provider
{
    ...
    public enum VsSelfStateIcon
    {
        STATEICON_ADDING = 12,
        STATEICON_COMMITTING = 13,
        STATEICON_MIXED = 14,
        STATEICON_STAGED_INREPOSITORY=15
    }


    [Guid("DD172D54-6B50-4D7D-BA5E-99F875A7C9AD")]
    public class SccProviderService : 
        CGitManager,                 //Git function Manager 
        ...
        IVsSccGlyphs,               //add NHN self status icon        
        ...
        IDisposable
    {
        ...
        // The Glyph Icon index for self icon
        private uint BaseStateIconIndex;


        // RDT
        uint m_nRdtCookie;
        RunningDocumentTable m_oRdTVSFrame;

        #region IVsRunningDocTableEvents Members
        public int OnAfterAttributeChange(uint docCookie, uint grfAttribs)
        {
            if (m_oRdTVSFrame == null || m_nRdtCookie == 0)
                return VSConstants.S_OK;
            RunningDocumentInfo rdi = m_oRdTVSFrame.GetDocumentInfo(docCookie);
            if (string.IsNullOrEmpty(rdi.Moniker))
                return VSConstants.S_OK;

            IList<VSITEMSELECTION> selectedNodes = new List<VSITEMSELECTION>();
            selectedNodes.Clear();
            VSITEMSELECTION vsItemSelection;
            vsItemSelection.pHier = rdi.Hierarchy;
            vsItemSelection.itemid = rdi.ItemId;
            selectedNodes.Add(vsItemSelection);
            _sccProvider.RefreshNodesGlyphs(selectedNodes);
            return VSConstants.S_OK;
        }
        ...
        public int SetActive()
        {
            ...
            //create a git object, it will supply all scc function;    --by fengzheng
            IVsUIShell uiShell = (IVsUIShell)_sccProvider.GetService(typeof(SVsUIShell));
            IntPtr hWnd = IntPtr.Zero;
            uiShell.GetDialogOwnerHwnd(out hWnd);
            Debug.Assert(hWnd != IntPtr.Zero);            
            if( false==MgrInitialize(hWnd))
            {
                Trace.WriteLine(String.Format("/nGit Manager initialize failed when Provider set active."));   //debug    --by fengzheng
        ...
        //As part of the process of switching source control providers, this method is called to determine
        //if the user should close a currently open solution before switching to another or if the solution can be closed automatically. 
        ...
        public int GetSccGlyph( [InAttribute] int cFiles, [InAttribute] string[] rgpszFullPaths, [OutAttribute] VsStateIcon[] rgsiGlyphs, [OutAttribute] uint[] rgdwSccStatus )
        {
            ...
            FileSccStatus status = GetFileStatus(rgpszFullPaths[0]);
            switch (status)
            {
                case FileSccStatus.ST_INVALID_REPO:
                    return VSConstants.S_FALSE;
                    break;

                case FileSccStatus.ST_NOT_CONTROLLED:
                    rgsiGlyphs[0] = (VsStateIcon)VsSelfStateIcon.STATEICON_ADDING;   
                    if (rgdwSccStatus != null)
                    {
                       rgdwSccStatus[0] = (uint)FileSccStatus.ST_NOT_CONTROLLED;                       
                       
                    }
                    break;
                case FileSccStatus.ST_IGNORED:
                    rgsiGlyphs[0] = VsStateIcon.STATEICON_BLANK;
                    if (rgdwSccStatus != null)
                    {
                        rgdwSccStatus[0] = (uint)FileSccStatus.ST_IGNORED;

                    }
                    break;
                case FileSccStatus.ST_NEW_CHECKIN:
                    rgsiGlyphs[0] = (VsStateIcon)VsSelfStateIcon.STATEICON_STAGED_INREPOSITORY;
                    if (rgdwSccStatus != null)
                    {
                        rgdwSccStatus[0] = (uint)FileSccStatus.ST_NEW_CHECKIN;
                    }
                    break;
                case FileSccStatus.ST_NEW_STAGED:
                    rgsiGlyphs[0] = (VsStateIcon)VsSelfStateIcon.STATEICON_COMMITTING;
                    if (rgdwSccStatus != null)
                    {
                        rgdwSccStatus[0] = (uint)FileSccStatus.ST_NEW_STAGED;
                    }
                    break;
                case FileSccStatus.ST_MODIFY_STAGED:
                    rgsiGlyphs[0] = (VsStateIcon)VsSelfStateIcon.STATEICON_COMMITTING;
                    if (rgdwSccStatus != null)
                    {
                        rgdwSccStatus[0] = (uint)FileSccStatus.ST_MODIFY_STAGED;
                    }
                    break;
               case FileSccStatus.ST_MODIFY_STAGED_MODIFY:
                    rgsiGlyphs[0] = (VsStateIcon)VsSelfStateIcon.STATEICON_MIXED;
                    if (rgdwSccStatus != null)
                    {
                        rgdwSccStatus[0] = (uint)FileSccStatus.ST_MODIFY_STAGED_MODIFY;
                    }
                    break;

                case FileSccStatus.ST_CHECKIN_MODIFIED:
                    rgsiGlyphs[0] = (VsStateIcon)VsSelfStateIcon.STATEICON_ADDING;               
                    if (rgdwSccStatus != null)
                    {
                        rgdwSccStatus[0] = (uint)FileSccStatus.ST_CHECKIN_MODIFIED;
                    }
                    break;

                case FileSccStatus.ST_STAGE_MODIFIED:
                    rgsiGlyphs[0] = (VsStateIcon)VsSelfStateIcon.STATEICON_MIXED;
                    if (rgdwSccStatus != null)
                    {
                        rgdwSccStatus[0] = (uint)FileSccStatus.ST_STAGE_MODIFIED;
                    }
                    break;
                case FileSccStatus.ST_CONFLICT:
                    rgsiGlyphs[0] = (VsStateIcon)VsSelfStateIcon.STATEICON_COMMITTING;
                    //rgsiGlyphs[0] = VsStateIcon.STATEICON_CHECKEDOUTEXCLUSIVEOTHER;
                    if (rgdwSccStatus != null)
                    {
                        rgdwSccStatus[0] = (uint)FileSccStatus.ST_CONFLICT;
                    }
                    break;
                case FileSccStatus.ST_DELETE:
                    //rgsiGlyphs[0] = VsStateIcon.STATEICON_CHECKEDOUTEXCLUSIVEOTHER;
                    rgsiGlyphs[0] = (VsStateIcon)VsSelfStateIcon.STATEICON_COMMITTING;
                    if (rgdwSccStatus != null)
                    {
                        rgdwSccStatus[0] = (uint)FileSccStatus.ST_DELETE;
                    }
                    break;
                case FileSccStatus.ST_NULL:
                    rgsiGlyphs[0] = VsStateIcon.STATEICON_DISABLED;   //this is maybe a error, check it.
                    if (rgdwSccStatus != null)
                    {
                        rgdwSccStatus[0] = (uint)FileSccStatus.ST_NULL;
                    }
                    break;
            }
        ...
        public int GetGlyphTipText([InAttribute] IVsHierarchy phierHierarchy, [InAttribute] uint itemidNode, out string pbstrTooltipText)
        {
            ...
            FileSccStatus status = GetFileStatus(files[0]);
            switch (status)
            {
                case FileSccStatus.ST_NOT_CONTROLLED:
                    pbstrTooltipText = Resources.ResourceManager.GetString("STATUS_SZ_NOT_CONTROLLED"); 
                    break;
                case FileSccStatus.ST_NEW_CHECKIN:
                    pbstrTooltipText = Resources.ResourceManager.GetString("STATUS_SZ_NEW_CHECKIN");
                    break;
                case FileSccStatus.ST_NEW_STAGED:
                    pbstrTooltipText = Resources.ResourceManager.GetString("STATUS_SZ_NEW_STAGED");
                    break;
                case FileSccStatus.ST_MODIFY_STAGED:
                    pbstrTooltipText = Resources.ResourceManager.GetString("STATUS_SZ_MODIFY_STAGED");
                    break;
                case FileSccStatus.ST_MODIFY_STAGED_MODIFY:
                    pbstrTooltipText = Resources.ResourceManager.GetString("STATUS_SZ_MODI_STAGED_MODI");
                    break;
                case FileSccStatus.ST_CHECKIN_MODIFIED:
                    pbstrTooltipText = Resources.ResourceManager.GetString("STATUS_SZ_CHECKIN_MODIFIED");
                    break;
                case FileSccStatus.ST_STAGE_MODIFIED:
                    pbstrTooltipText = Resources.ResourceManager.GetString("STATUS_SZ_STAGE_MODIFIED");
                    break;
                case FileSccStatus.ST_IGNORED:
                    pbstrTooltipText = Resources.ResourceManager.GetString("STATUS_SZ_IGNORED");
                    break;
                case FileSccStatus.ST_DELETE:
                    pbstrTooltipText = Resources.ResourceManager.GetString("STATUS_SZ_DELETE");
                    break;
                case FileSccStatus.ST_RENMAE:
                    pbstrTooltipText = Resources.ResourceManager.GetString("STATUS_SZ_RENMAE");
                    break;
                case FileSccStatus.ST_CONFLICT:
                    pbstrTooltipText = Resources.ResourceManager.GetString("STATUS_SZ_CONFLICT");
                    break;

                default:
                    pbstrTooltipText = Resources.ResourceManager.GetString("Unkown Status");
                    break;
            }
        ...
        public int QueryEditFiles([InAttribute] uint rgfQueryEdit, [InAttribute] int cFiles, [InAttribute] string[] rgpszMkDocuments, [InAttribute] uint[] rgrgf, [InAttribute] VSQEQS_FILE_ATTRIBUTE_DATA[] rgFileInfo, out uint pfEditVerdict, out uint prgfMoreInfo)
        {
            ...
            try 
            {
                ...
                for (int iFile = 0; iFile < cFiles; iFile++)
                {
                    ...
                    FileSccStatus status = GetFileStatus(rgpszMkDocuments[iFile]);
                        ...
                        else
                        {
                            switch (status)
                            {
                                case FileSccStatus.ST_NEW_CHECKIN:
                                    ...
                                    break;
                                case FileSccStatus.ST_DELETE: // fall through
                                case FileSccStatus.ST_NULL:
                                    if (fileExists && isFileReadOnly)
                                    {
                                        if ((rgfQueryEdit & (uint)tagVSQueryEditFlags.QEF_ReportOnly) != 0)
                ...
                for (int iFile = 0; iFile < cFiles; iFile++)
                {
                    ...
                    switch (status)
                    {
                        case FileSccStatus.ST_NEW_STAGED:
                            ...
                            break;
                        case FileSccStatus.ST_NEW_CHECKIN: // fall through
                        case FileSccStatus.ST_DELETE:
                            if (fileExists && isFileReadOnly)
                            {
        ...
        public int GetCustomGlyphList( uint BaseIndex, out uint pdwImageListHandle )
         {
            ...
            ImageList GitImageList = new ImageList();
            GitImageList.TransparentColor = System.Drawing.Color.White;
            GitImageList.ImageSize = new System.Drawing.Size(7, 16);

            //GitImageList.Images.Add(((System.Drawing.Image)(Resources.ResourceManager.GetObject("resource"))));
            GitImageList.Images.Add(((System.Drawing.Image)(Resources.Icon_State_Add)));
            GitImageList.Images.Add(((System.Drawing.Image)(Resources.Icon_State_Commit)));
            GitImageList.Images.Add(((System.Drawing.Image)(Resources.Icon_State_Mix)));
            GitImageList.Images.Add(((System.Drawing.Image)(Resources.Icon_State_Repo)));


            this.BaseStateIconIndex=BaseIndex;
            pdwImageListHandle = (uint)GitImageList.Handle.ToInt32();
            return VSConstants.S_OK;
         }
