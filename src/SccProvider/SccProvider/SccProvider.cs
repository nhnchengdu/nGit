...
namespace Nhn.Git.SourceControl.Provider
{
    ...
    [MsVsShell.ProvideLoadKey("Standard", "1.0.0.0", "NHN Git Source Control Provider", "NHN", 102)]
    ...
    [MsVsShell.ProvideOptionPageAttribute(typeof(SccProviderOptions), "Source Control", "NHN Git Options Page", 106, 107, false)]
    [ProvideToolsOptionsPageVisibility("Source Control", "NHN Git Options Page", "DD172D54-6B00-4D7D-BA5E-99F875A7C9AD")]
    ...
    [MsVsShell.ProvideToolWindowVisibility(typeof(SccProviderToolWindow), "DD172D54-6B00-4D7D-BA5E-99F875A7C9AD")]
    // Register the source control provider's service (implementing IVsScciProvider interface)
    [MsVsShell.ProvideService(typeof(SccProviderService), ServiceName = "NHN Git Source Control Provider Service")]
    // Register the source control provider to be visible in Tools/Options/SourceControl/Plugin dropdown selector
    [ProvideSourceControlProvider("NHN Git Source Control Provider", "#100")]
    ...
    [MsVsShell.ProvideAutoLoad("DD172D54-6B00-4D7D-BA5E-99F875A7C9AD")]
    ...
    [Guid("DD172D54-6B60-4D7D-BA5E-99F875A7C9AD")]
    public sealed partial class SccProvider : MsVsShell.Package,
        IOleCommandTarget,
        IVsPersistSolutionProps    // We'll write properties in the solution file to track when solution is controlled; the interface needs to be implemented by the package object
    {
        ...
        private const string _strProviderName = "NHN Git Source Control Provider:{DD172D54-6B60-4D7D-BA5E-99F875A7C9AD}";
        // The name of the solution section used to persist provider options (should be unique)
        private const string _strSolutionPersistanceKey = "NHNGitProviderSolutionProperties";
        ...
        private const string _strSolutionUserOptionsKey = "NHNGitSourceControlProvider";
        ...
        protected override void Initialize()
        {
            ...
            //((IServiceContainer)this).AddService(typeof(SVsIntellisenseEngine), sccService, true); ///xxxxxxxxxxxxxxxxxxxxxxxxx
            ...
            if(mcs != null)
            {
                ...
                //NGit Command binding              --by fengzheng
                cmd = new CommandID(GuidList.guidSccProviderCmdSet, CommandId.cmdidNhnGitCreate);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdNhnGitInitRepo), cmd);
                mcs.AddCommand(menuCmd);
                cmd = new CommandID(GuidList.guidSccProviderCmdSet, CommandId.cmdidNhnGitClone);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdNhnGitCloneRepo), cmd);
                mcs.AddCommand(menuCmd);

                cmd = new CommandID(GuidList.guidSccProviderCmdSet, CommandId.cmdidNhnGitStage);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdStage), cmd);
                mcs.AddCommand(menuCmd);
                cmd = new CommandID(GuidList.guidSccProviderCmdSet, CommandId.cmdidNhnGitCommit);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdNhnGitCommit), cmd);
                mcs.AddCommand(menuCmd);
                cmd = new CommandID(GuidList.guidSccProviderCmdSet, CommandId.cmdidNhnGitUnStage);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdUnStage), cmd);
                mcs.AddCommand(menuCmd);
                cmd = new CommandID(GuidList.guidSccProviderCmdSet, CommandId.cmdidNhnGitDiscard);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdNhnGitDiscard), cmd);
                mcs.AddCommand(menuCmd);
                cmd = new CommandID(GuidList.guidSccProviderCmdSet, CommandId.cmdidNhnGitDelete);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdNhnGitDelete), cmd);
                mcs.AddCommand(menuCmd);
                        

                cmd = new CommandID(GuidList.guidSccProviderCmdSet, CommandId.cmdidNhnGitRepo);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdNhnGitRepo), cmd);
                mcs.AddCommand(menuCmd);
                cmd = new CommandID(GuidList.guidSccProviderCmdSet, CommandId.cmdidNhnGitRemote);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdNhnGitRemote), cmd);
                mcs.AddCommand(menuCmd);
                cmd = new CommandID(GuidList.guidSccProviderCmdSet, CommandId.cmdidNhnGitLocal);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdNhnGitLocal), cmd);
                mcs.AddCommand(menuCmd);


                cmd = new CommandID(GuidList.guidSccProviderCmdSet, CommandId.cmdidNhnGitAdvanced);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdNhnGitAdvanced), cmd);
                mcs.AddCommand(menuCmd);
                cmd = new CommandID(GuidList.guidSccProviderCmdSet, CommandId.cmdidNhnGitBlame);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdNhnGitBlame), cmd);
                mcs.AddCommand(menuCmd);
                cmd = new CommandID(GuidList.guidSccProviderCmdSet, CommandId.cmdidNhnGitIgnore);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdNhnGitIgnore), cmd);
                mcs.AddCommand(menuCmd);
                cmd = new CommandID(GuidList.guidSccProviderCmdSet, CommandId.cmdidNhnProperyWnd);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdGitPropertylWindow), cmd);
                mcs.AddCommand(menuCmd);
                cmd = new CommandID(GuidList.guidSccProviderCmdSet, CommandId.cmdidNhnGitStash);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdNhnGitStash), cmd);
                mcs.AddCommand(menuCmd);
            }
        ...
        public int QueryStatus(ref Guid guidCmdGroup, uint cCmds, OLECMD[] prgCmds, System.IntPtr pCmdText)
        {
            ...
            switch(prgCmds[0].cmdID)
            {
                ...
                //NGit Funtion GUI  Enable     --by fengzheng 
                case CommandId.cmdidNhnGitCreate:
                case CommandId.cmdidNhnGitClone:
                case CommandId.cmdidNhnGitStage:
                case CommandId.cmdidNhnGitUnStage:
                case CommandId.cmdidNhnGitCommit:
                case CommandId.cmdidNhnGitDiscard:
                case CommandId.cmdidNhnGitDelete:
                case CommandId.cmdidNhnGitRepo:
                case CommandId.cmdidNhnGitRemote:
                case CommandId.cmdidNhnGitLocal:
                case CommandId.cmdidNhnGitStash:
                case CommandId.cmdidNhnGitAdvanced:
                case CommandId.cmdidNhnGitBlame:
                case CommandId.cmdidNhnGitIgnore:
                case CommandId.cmdidNhnProperyWnd:
        ...
        /// <summary>
        /// Returns the directory of the solution
        /// </summary>
        ...
        private void ShowToolWindow()
        {
            ...
            SccProviderToolWindow PropertWindow = (SccProviderToolWindow)this.FindToolWindow(typeof(SccProviderToolWindow), 0, true);
            if (window != null)
            {
                PropertWindow.Initialize(sccService);
            }
        }
        ...
        public void ValidConfig()
        {
            ...
            if (window != null)
            {
                window.ValidConfig(this.GetSolutionDirectory());
            }
        }
